...
const stableSetActive = async () => undefined;
const stableTicketSignIn = {
  create: async () => ({
    status: "complete",
    createdSessionId: "test_session_1",
  }),
};
...
describe("public sitemap routes", () => {
  ...
  it("renders every sitemap URL with JavaScript enabled", async () => {
    for (const route of publicRoutes) {
      const view = await renderApp(route.path);

      if (route.path === "/shop") {
        expect(
          (await screen.findAllByText(route.content, undefined, {
            timeout: 5000,
          })).length,
          `${route.path} should render its public content`,
        ).toBeGreaterThan(0);
      } else {
        expect(
          await screen.findByRole(
            "heading",
            { name: route.content, level: 1 },
            { timeout: 5000 },
          ),
          `${route.path} should render its public H1`,
        ).toBeInTheDocument();
      }
      expect(
        screen.queryByRole("heading", { name: /404 Page Not Found/i }),
        `${route.path} must not fall through to Not Found`,
