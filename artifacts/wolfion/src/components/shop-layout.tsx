...
export function ShopLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-white text-neutral-900 dark:bg-neutral-950 dark:text-neutral-50 antialiased font-sans">
      {/* Editorial top bar */}
      <header className="sticky top-0 z-50 w-full bg-white/85 dark:bg-neutral-950/85 backdrop-blur-md border-b border-neutral-200/70 dark:border-neutral-800/70">
        <div className="container mx-auto px-5 h-16 flex items-center justify-between">
          ...
          {/* Center logo */}
          <Link href="/shop" className="md:absolute md:left-1/2 md:-translate-x-1/2 flex items-center gap-2.5">
            ...
            <span className="font-semibold tracking-[0.4em] text-base sm:text-lg">WOLFION</span>
          </Link>

          {/* Desktop nav (left) */}
          <nav className="hidden md:flex items-center gap-7 absolute left-6">
            {NAV.map((n) => {
              ...
              <Sheet>
                ...
                <SheetContent side="right" className="w-[320px] p-0 flex flex-col bg-white dark:bg-neutral-950">
                  <SheetHeader className="px-6 pt-7 pb-5 text-left border-b border-neutral-200 dark:border-neutral-800">
                    ...
                    <p className="text-xs text-neutral-500 truncate">{user?.fullName || user?.emailAddresses[0]?.emailAddress}</p>
                  </SheetHeader>
