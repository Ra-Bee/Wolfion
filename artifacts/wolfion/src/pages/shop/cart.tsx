        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-10 lg:gap-16 items-start">
          ...
          <aside className="lg:sticky lg:top-24">
            <GlassCard
              ...
              ringed={false}
            >
            <h2 className="text-[10px] uppercase tracking-[0.4em] font-medium text-neutral-500 mb-6">Order Summary</h2>

            <div className="space-y-4 text-sm font-light">
              <div className="flex justify-between"><span className="text-neutral-500">Subtotal</span><span className="font-serif italic text-base">Tk {totalPrice.toFixed(2)}</span></div>
              <div className="flex justify-between">
                <span className="text-neutral-500">Shipping</span>
                <span className="font-serif italic text-base">{shipping === 0 ? "Free" : `Tk ${shipping.toFixed(2)}`}</span>
              </div>
              <div className="flex justify-between"><span className="text-neutral-500">Estimated tax</span><span className="font-serif italic text-base">Tk {tax.toFixed(2)}</span></div>
            </div>

            <div className="border-t border-neutral-200 dark:border-white/10 mt-6 pt-6 flex justify-between items-baseline">
              <span className="text-sm uppercase tracking-widest font-medium">Total</span>
              <span className="text-3xl font-serif italic font-light">Tk {grandTotal.toFixed(2)}</span>
            </div>

            <Button
              className="w-full mt-8 h-14 rounded-full bg-black text-white hover:bg-neutral-800 dark:bg-white dark:text-black dark:hover:bg-neutral-200 text-[10px] uppercase tracking-[0.2em] font-medium py-4 shadow-[0_4px_14px_rgba(0,0,0,0.2)]"
              onClick={() => setPayOpen(true)}
              data-testid="checkout"
            >
              Checkout <ArrowRight className="ml-3 h-4 w-4" />
            </Button>
