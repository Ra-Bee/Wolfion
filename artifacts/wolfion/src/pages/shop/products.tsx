...
> = {
  ...
  kids: {
    eyebrow: "Collection",
    title: "Kidswear",
    subtitle: "Soft, durable, and built to play.",
  },
};
...
export default function Products() {
  ...
  const activeCategory =
    (params.get("category") as ProductCategory | null) ?? null;
  const activeCollection =
    (params.get("collection") as ProductCollection | null) ?? null;
    ...
    return products.filter((p) => {
      ...
      if (activeCollection) {
        if (activeCollection === "kids") {
          if (p.collection !== "kids") return false;
        } else if (p.collection !== activeCollection && p.collection !== "unisex") {
          return false;
        }
      }
      return true;
  ...
  const collectionMeta = activeCollection
    ? COLLECTION_META[activeCollection]
    : null;
  ...
  // Build query helper preserving collection
  const buildHref = (categoryId: ProductCategory | null) => {
    const sp = new URLSearchParams();
    if (categoryId) sp.set("category", categoryId);
    if (activeCollection) sp.set("collection", activeCollection);
    ...
    <ShopLayout>
      ...
      <section className="relative bg-gradient-to-b from-neutral-50 to-white dark:from-neutral-900 dark:to-neutral-950 border-b border-neutral-200/70 dark:border-neutral-800/70">
        <div className="container mx-auto px-5 pt-14 pb-10">
          ...
          {subtitle && (
            <p className="mt-4 max-w-xl text-sm text-neutral-500 font-light leading-relaxed">
              {subtitle}
            </p>
          )}
        </div>
      ...
      <section className="container mx-auto px-5 pt-8 pb-20">
        ...
        <div className="flex flex-wrap gap-2 mb-10 pb-5">
          ...
          {categories.map((c) => (
            <Link key={c.id} href={buildHref(c.id)}>
              <button
                className={`px-4 h-9 rounded-full text-xs uppercase tracking-widest transition-all active:scale-95 ${
                  activeCategory === c.id
                    ? "bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 shadow-md"
        ...
        {filtered.length === 0 ? (
          <div className="py-20 text-center text-neutral-500">
            ...
            <Link href="/products" className="underline text-sm mt-3 inline-block">
              View all
            </Link>
          </div>
        ) : (
