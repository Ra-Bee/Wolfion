...
export default function Products() {
  ...
  return (
    <ShopLayout>
      {/* Hero band */}
      <section className="relative border-b border-black/5 dark:border-white/10 mt-16 md:mt-24">
        <div className="container mx-auto px-5 pt-8 pb-10">
          <p className="text-[11px] uppercase tracking-[0.4em] text-neutral-500 mb-4">{eyebrow}</p>
          <h1 className="text-4xl sm:text-6xl font-serif italic tracking-wide text-black dark:text-white">{title}</h1>
          {subtitle && (
            <p className="mt-5 max-w-xl text-sm sm:text-base text-neutral-500 font-light leading-relaxed">{subtitle}</p>
          )}
        </div>
      </section>
      ...
      <section className="container mx-auto px-5 pt-8 pb-20">
        ...
        <div className="flex flex-wrap gap-2 mb-10 pb-5">
          <Link href="/products">
            <button
              className={`relative px-5 h-10 rounded-full text-[10px] uppercase tracking-[0.2em] font-medium transition-all active:scale-95 border ${
                isAll
                  ? "bg-black text-white dark:bg-white dark:text-black border-transparent shadow-[0_4px_14px_rgba(0,0,0,0.2)]"
                  : "text-neutral-500 border-neutral-200 dark:border-white/10 hover:bg-neutral-100 dark:hover:bg-white/5"
              }`}
