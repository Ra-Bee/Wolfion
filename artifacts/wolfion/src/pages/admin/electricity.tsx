...
import { Zap, Plus, Battery, Gauge } from "lucide-react";
...
export default function ElectricityPage() {
  ...
  const [production] = useStored<ProductionEntry[]>(STORAGE_KEYS.production, []);
  ...
  // divided by dozens produced in that same period. Windows are rolling from
  // today: weekly = last 7 days, monthly = last 30 days, yearly = last 365 days.
  const avgPerDozen = useMemo(() => {
    const now = Date.now();
    ...
    return windows.map((w) => {
      ...
      const spend = recharges.reduce((s, r) => {
        const t = new Date(r.date).getTime();
        return Number.isFinite(t) && t >= cutoff ? s + (Number(r.amount) || 0) : s;
      }, 0);
      const dozens = production.reduce((s, p) => {
        const t = new Date(p.date).getTime();
        return Number.isFinite(t) && t >= cutoff ? s + (Number(p.quantityDozen) || 0) : s;
      }, 0);
  ...
  return (
    <AdminLayout>
      <div className="container mx-auto px-4 py-8 max-w-5xl space-y-6">
        ...
        <Card className="border shadow-md">
          <CardHeader>
            <CardTitle>Electricity Card Recharge</CardTitle>
        ...
        <Card className="border shadow-md">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Gauge className="h-5 w-5 text-primary" /> Avg electricity cost per dozen
            </CardTitle>
            <CardDescription>
              Electricity recharged ÷ dozens produced. Rolling windows: weekly = last 7 days, monthly = last 30 days, yearly = last 365 days.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* Big, clear per-dozen tiles — the number the user cares about. */}
              ...
              {avgPerDozen.map((p) => (
                <div key={p.label} className="rounded-xl border-2 border-primary/30 bg-primary/5 p-3 text-center">
                  ...
                  <p className="mt-1 text-lg font-bold tabular-nums leading-tight">
                    {p.dozens > 0 ? money(p.perDozen) : "—"}
                  </p>
                  <p className="text-[10px] text-muted-foreground">per dozen</p>
                </div>
              ))}
            </div>
            {/* Detail behind each figure: electricity spent ÷ dozens produced. */}
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="text-left text-muted-foreground border-b">
                  <tr>
                    <th className="py-2 pr-3 font-medium">Period</th>
                    <th className="py-2 pr-3 text-right font-medium">Electricity</th>
                    <th className="py-2 pr-3 text-right font-medium">Dozens</th>
                    <th className="py-2 text-right font-medium">Cost / dz</th>
                  </tr>
                </thead>
                <tbody>
                  {avgPerDozen.map((p) => (
                    <tr key={p.label} className="border-b last:border-0">
                      ...
                      <td className="py-2 pr-3 text-right tabular-nums">{money(p.spend)}</td>
                      <td className="py-2 pr-3 text-right tabular-nums">{p.dozens.toLocaleString(undefined, { maximumFractionDigits: 2 })} dz</td>
                      <td className="py-2 text-right font-bold tabular-nums">{p.dozens > 0 ? money(p.perDozen) : "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            {avgPerDozen.every((p) => p.dozens === 0) && (
              <p className="text-xs text-muted-foreground">
                No production recorded in these periods yet — add production entries to see the average cost per dozen.
              </p>
            )}
          </CardContent>
        </Card>
