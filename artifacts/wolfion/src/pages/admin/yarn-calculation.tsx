...
export default function YarnCalculationPage() {
  ...
  const futureRows = useMemo(() => {
    return allYarns.map((item) => {
      const row = typeRows.find((t) => t.id === item.id);
      const remaining = row?.remaining ?? 0;
      const weeklyNeed = needByYarn.weekly[item.id] || 0;
      const monthlyNeed = needByYarn.monthly[item.id] || 0;
      const coverage = monthlyNeed > 0 ? remaining / monthlyNeed : remaining > 0 ? 1 : 0;
      const weeksLeft = weeklyNeed > 0 ? remaining / weeklyNeed : 0;
      const shortfall = monthlyNeed - remaining;
      return { id: item.id, label: item.label, remaining, weeklyNeed, monthlyNeed, coverage, weeksLeft, shortfall };
    });
  }, [allYarns, typeRows, needByYarn]);
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl space-y-6">
        ...
        <Card className="border-2 border-primary/30 shadow-md">
              ...
              <div>
                ...
                <CardDescription>
                  {totalAvgDozenPerDay > 0
                    ...
                    : "Record some daily production and this will auto-calculate how much yarn you need each week and month."}
                </CardDescription>
              </div>
              <div className="flex rounded-full border p-1 shrink-0">
                <button
                  type="button"
                  onClick={() => setPeriod("weekly")}
                  className={`px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${period === "weekly" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
                >Weekly</button>
                <button
                  type="button"
                  onClick={() => setPeriod("monthly")}
                  className={`px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${period === "monthly" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
                >Monthly</button>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-5">
            {futureRows.map((r) => {
              const need = period === "weekly" ? r.weeklyNeed : r.monthlyNeed;
              ...
              const periodWordPlural = period === "weekly" ? "weeks" : "months";
              return (
                <div key={r.id} className="space-y-1.5">
                  ...
                  <div className="text-xs tabular-nums">
                    {need <= 0 ? (
                      <span className="text-muted-foreground">Record daily production to see {period} need</span>
                    ) : have <= 0 ? (
                ...
                <tbody>
                  {typeRows.map((r) => (
                    <tr key={r.id} className="border-b last:border-0">
                      ...
                      <td className="py-3 text-right tabular-nums">{money(r.pricePerKg)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
