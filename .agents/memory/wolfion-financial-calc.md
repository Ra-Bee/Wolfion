...
## Cloud-stored data can violate the TS shape — guard/normalize at boundaries
Persisted `productConfig` (and similar cloud/local records) may be missing fields the
TS type claims exist — e.g. legacy configs with no `yarnRecipe`. TypeScript can't catch
this; it surfaces as a runtime crash like `yarnRecipe is not iterable` that takes down
the whole admin screen.
**Rule:** never iterate/spread/`.length`/`.map` a persisted array field directly; read it
as `x.field ?? []` (or optional-chain), or normalize records at the load boundary.
Re-saving a record through its editor form re-normalizes its stored shape.
**Why:** stored data outlives the current schema; a single unguarded access is a
production outage.
